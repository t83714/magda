...
HELM_CONFIG_FILE=magda-dev-kn-v0.0.41.yml
...
then
    ...
    # Install using helm
    cd /app/deploy/helm && helm install --timeout 999999999 --name $NAMESPACE magda  --set web-server.image.repository="gcr.io/knowledge-network-prod/data61" --set web-server.image.tag=$TAG --set gateway.image.repository="gcr.io/knowledge-network-prod/data61" --set gateway.image.tag=$TAG -f magda-prod-kn-slim.yml
