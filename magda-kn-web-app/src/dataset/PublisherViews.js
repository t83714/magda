...
export default class PublisherViews extends Component {
    constructor(props){
        super(props)
        this.state = { totalCount: 0, viewType:'line', publisherMap:new Map(), datasetInfo:[]}
        //view type: line, grid
    ...
    dropDownItemChange (e) {
        if(e==='All'){
            ...
            this.setState({dropdownTitle: 'All Datasources'})
        }else{
            this.allPublisher = this.props.publisherMap.get(e)
            this.setState({dropdownTitle: this.props.datasource.get(e).name})
        }   
    }
    ...
    render(){
        ...
        const lineView = display.map((value, key) => {
            return (
        ...
        const gridView = display.map((value, key) => {
            return (
            ...
            <Row>
                <i> Find total {this.allPublisher.length} publishers in {this.props.default==='All'? this.props.datasource.size + ' datasources' : ' datasource'}  &nbsp;
                    ...
                    {datasourceKeys.map((value, key)=>{
                        if(value===this.props.default) return (<MenuItem eventKey={value} active key={key+1} onSelect={this.dropDownItemChange} >{this.props.datasource.get(value).name}</MenuItem>)
                        else return (<MenuItem eventKey={value} key={key+1} onSelect={this.dropDownItemChange}>{this.props.datasource.get(value).name}</MenuItem>)
                    })}
                    </DropdownButton>
