...
export default class Pagination extends Component {
    ...
    handlePageOffsetBack(){
        var temp = this.state.pageOffset - this.props.perPage*this.state.defaultPages
        ...
        if(this.props.total>this.props.perPage){
            return(
                ...
                <ul className="pagination">
                    ...
                    {Array(this.props.total).fill().map( (_, i) => { 
                        // Display two button for navigation by default
                        if( i>=this.state.pageOffset && i<=this.state.pageOffset+this.props.perPage*this.state.defaultPages && i%this.props.perPage === 0) {
                            if( i/this.props.perPage === this.state.currentPage) return (<li className="active" key={i}> <a>{i/this.props.perPage + 1} </a></li>)
                            else return (<li onClick={() => this.handlePageNavClick(i)} key={i}> <a>{i/this.props.perPage +1} </a></li>)
                        }else return ("")
                        })}
                    {this.state.pageOffset + this.props.perPage*this.state.defaultPages < this.props.total ?
